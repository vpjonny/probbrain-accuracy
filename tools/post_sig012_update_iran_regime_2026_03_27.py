"""
Post SIG-012 UPDATE: Will the Iranian regime fall before 2027? (NO_UNDERPRICED)
Sourced from PRO-286 (2026-03-27T14:15:00Z scan) — updated market data.
Content reviewed by Content Creator (PRO-287) and Twitter Engager (PRO-288).

Signal Confidence Rule applied: gap 15.0pp < 18% on a >6-month horizon (closes 2026-12-31)
→ badge downgraded from HIGH to MEDIUM per AGENTS.md rules.

Note: SIG-012 was first published 2026-03-26. This is an update with refreshed market prices.

This script:
1. Posts SIG-012 update to Telegram then X
2. Updates data/published_signals.json
3. Syncs dashboard
4. PATCHes PRO-286 to done via Paperclip API

Usage: python tools/post_sig012_update_iran_regime_2026_03_27.py [--dry-run]
"""
import json
import logging
import os
...
PRO_286_ID = "298e7ea1-1a18-4ef4-bbc9-341ffa4af386"

DRY_RUN = "--dry-run" in sys.argv

# Final copy — updated numbers from 2026-03-27 scan.
# Badge downgraded from HIGH → MEDIUM: gap 15.0pp < 18% on >6-month horizon (closes 2026-12-31).
# Reviewed by Content Creator (PRO-287) and Twitter Engager (PRO-288).
SIGNAL = {
    "signal_number": 12,
    "signal_id": "SIG-012",
    ...
    "confidence": "MEDIUM",
    "our_estimate": 0.20,
    "market_price_at_signal": 0.35,
    "gap_pct": 15.0,
    "volume_usdc": 11800000,
    "close_date": "2026-12-31",
    "paperclip_issue": "PRO-286",
    "evidence": [
    ...
    "telegram_copy": (
        ...
        "Market: 35% YES | Our estimate: 20% YES\n"
        "Gap: 15.0pp (market overpricing YES)\n"
        "Volume: $11.8M\n"
        "Closes: 2026-12-31\n\n"
        "Evidence:\n"
        "\u2022 Khamenei survived the Feb 28 assassination attempt \u2014 hardliners consolidated power, succession not disrupted\n"
    ...
    "x_thread_copy": {
        "tweet_1": (
            "Polymarket prices the Iranian regime falling before 2027 at 35%. "
            "Khamenei survived assassination in Feb. Mojtaba installed March 9 \u2014 IRGC intact. "
            "Our estimate: 20%. Gap: 15pp. [thread]"
        ),
        "tweet_2": (
...
def update_published(sig: dict, tg_result: dict, x_ids: list, posted_at: str):
    ...
    entry = {
        ...
        "approved_by": f"auto-{sig['paperclip_issue']}",
    ...
    logger.info("published_signals.json updated for SIG-012 (update)")


def sync_dashboard(signal_id: str):
    import subprocess
    sync_script = BASE / "tools" / "sync_dashboard.py"
    if sync_script.exists():
        result = subprocess.run(
            ["python3", str(sync_script), f"--signal-id={signal_id}"],
            capture_output=True,
            text=True,
            cwd=str(BASE),
        )
        if result.returncode == 0:
            logger.info("Dashboard synced for %s", signal_id)
        else:
            logger.warning("Dashboard sync stderr: %s", result.stderr[:300])
    else:
        # Fallback to git_push_dashboard.py
        fallback = BASE / "tools" / "git_push_dashboard.py"
        if fallback.exists():
            subprocess.run(
                ["python3", str(fallback)],
                capture_output=True, text=True, cwd=str(BASE),
            )
            logger.info("Dashboard pushed via git_push_dashboard.py")


def paperclip_update(status: str, comment: str):
    if not PAPERCLIP_API_URL or not PAPERCLIP_API_KEY:
        logger.warning("Paperclip env vars not set — skipping update")
        return
    url = f"{PAPERCLIP_API_URL}/api/issues/{PRO_286_ID}"
    headers = {
    ...
    with httpx.Client(timeout=20) as client:
        ...
        if resp.status_code == 200:
            logger.info("PRO-286 updated to %s in Paperclip", status)
        else:
            logger.error("Paperclip PATCH failed: %s %s", resp.status_code, resp.text)


def main():
    logger.info("=== SIG-012 UPDATE posting script starting (DRY_RUN=%s) ===", DRY_RUN)
    ...
    # Telegram
    logger.info("Posting SIG-012 update to Telegram...")
    if DRY_RUN:
    ...
    # X thread
    logger.info("Posting SIG-012 update X thread...")
    if DRY_RUN:
        logger.info(
            "DRY RUN X:\nTweet 1: %s\nTweet 2: %s\nTweet 3: %s",
    ...
    logger.info("SIG-012 update posted at %s", posted_at)

    # Sync dashboard
    logger.info("Syncing dashboard...")
    if not DRY_RUN:
        sync_dashboard("SIG-012")

    summary = (
        "## SIG-012 UPDATE published\n\n"
        "SIG-012 update posted to Telegram (@ProbBrain) and X (@ProbBrain).\n\n"
        "**Signal:** Will the Iranian regime fall before 2027?\n"
        "**Direction:** NO_UNDERPRICED | **Confidence:** MEDIUM\n"
        "(Downgraded from HIGH: gap 15.0pp < 18% on >6-month horizon per agent rules.)\n"
        "**Market:** 35% YES | **Our estimate:** 20% YES | **Gap:** 15.0pp\n"
        "**Volume:** $11.8M | **Closes:** 2026-12-31\n\n"
        "Copy reviewed by Content Creator ([PRO-287](/PRO/issues/PRO-287)) "
        "and Twitter Engager ([PRO-288](/PRO/issues/PRO-288)).\n"
        "Auto-published (gap < 20pp, approval_required: false).\n\n"
        "Dashboard synced. Not financial advice."
    )
    if not DRY_RUN:
        paperclip_update("done", summary)
    else:
        logger.info("DRY RUN — would mark PRO-286 done with:\n%s", summary)
