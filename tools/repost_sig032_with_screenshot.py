#!/usr/bin/env python3
"""Delete and re-post SIG-032 with market card screenshot and hashtags"""
...
def delete_old_tweets():
    """Delete the old tweets without screenshot."""
    if not all([X_CONSUMER_KEY, X_CONSUMER_SECRET, X_ACCESS_TOKEN, X_ACCESS_TOKEN_SECRET]):
        print("⚠️ X credentials not set; skipping deletion")
        return False
    ...
    print("🗑️ Deleting old tweets...")
    for tweet_id in OLD_TWEET_IDS:
        try:
            client.delete_tweet(id=tweet_id)
            print(f"  ✓ Deleted tweet {tweet_id}")
        except Exception as e:
            print(f"  ⚠️ Could not delete tweet {tweet_id}: {e}")

    return True

def build_x_thread():
    """Build X thread tweets with hashtags."""
    # Tweet 1: Main hook
...
def post_to_x(tweet_1, tweet_2, tweet_3):
    ...
    # Post thread
    print("📤 Posting new thread with market card...")
    if media_id:
...
def update_published_signals(x_tweet_ids):
    """Update published_signals.json with new tweet IDs."""
    ...
    # Find the SIG-032 entry and update it with new tweet IDs
    for entry in published:
        if entry.get("signal_id") == SIGNAL_ID:
            entry["x_tweet_ids"] = list(x_tweet_ids)
            entry["edited_at"] = datetime.utcnow().isoformat() + "Z"
            break

    with open(published_file, "w") as f:
        json.dump(published, f, indent=2)

    print(f"✓ Updated published_signals.json with new tweet IDs")

def main():
    print(f"Re-posting {SIGNAL_ID}: {MARKET_QUESTION}")
    print("=" * 60)

    # Build messages
    x_tweets = build_x_thread()

    print("\n🐦 X thread (with hashtags):")
    print("-" * 60)
    ...
    # Delete old tweets and post new ones
    delete_old_tweets()
    x_ids = post_to_x(*x_tweets)

    # Update published signals
    if x_ids:
        update_published_signals(x_ids)
